...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrJoints(-1), attrWeights(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifCamPos(-1), unifBindMatrices(-1), unifJointTrans(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrJoints = context->glGetAttribLocation(prog, "vs_Joints");
    ...
    unifBindMatrices = context->glGetUniformLocation(prog, "u_BindMat");
    unifJointTrans = context->glGetUniformLocation(prog, "u_JointTrans");
}
...
//These set methods are used to send data to the glsl files!
...
void ShaderProgram::setJointTrans(const glm::mat4 jointTrans [100], int jTCount) {
    useMe();
    if (unifJointTrans != -1) {
        context->glUniformMatrix4fv(unifJointTrans,
                                    jTCount,
                                    GL_FALSE,
                                    &jointTrans[0][0][0]);
    }
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrJoints != -1 && d.bindPos()) {
        context->glEnableVertexAttribArray(attrJoints);
        context->glVertexAttribIPointer(attrJoints, 1, GL_INT, 0, nullptr);
        //context->glVertexAttribPointer(attrJoints, 1, GL_FLOAT, false, 0, nullptr);
    }

    if (attrWeights != -1 && d.bindPos()) {
        context->glEnableVertexAttribArray(attrWeights);
        context->glVertexAttribPointer(attrWeights, 1, GL_FLOAT, false, 0, nullptr);
    }
    ...
    if (attrJoints != -1) context->glDisableVertexAttribArray(attrJoints);
